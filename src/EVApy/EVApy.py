import datetime
import typing
import EVApy.EVAerrors as EVAerrors
...
"""
   ...
   ├─────────────────────────────────────────────────────┴────────────────┐
   │ import asyncio                                                       │
   │ import EVApy.EVApy as eva                                            │
   │                                                                      │
   │ async def main():                                                    │
   │     stats = await eva.getStats(username='EVA_USERNAME#12345')        │
   │     last_games = await eva.getLastGames(32123, 1, 15, items_limit=5) │
   │     location = await eva.getLocation(15)                             │
                ...
                try:
                    ...
                    if e.errors:
                        if e.errors[0]["message"] == 'User profile is private':
                            raise EVAerrors.UserIsPrivate() from None
                        elif e.errors[0]["message"] == "User not found":
                            raise EVAerrors.UserNotFound() from None
                        else:
                            raise
                except:
                    raise
